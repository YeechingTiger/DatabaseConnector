...
#' }
#' @export
queryMultiSql <- function(connection,
                          sql,
                          errorReportFile = file.path(getwd(), "errorReportSql.txt"),
                          snakeCaseToCamelCase = FALSE,
                          integerAsNumeric = getOption("databaseConnectorIntegerAsNumeric", default = TRUE),
  ...
  for (i in seq_along(sqlStatements)) {
    result <- tryCatch(
      {
        if (i == length(sqlStatements)) {
          # Execute the last statement using lowLevelQuerySql
          tempResult <- lowLevelQuerySql(
            connection = connection,
          ...
          if (snakeCaseToCamelCase) {
            colnames(tempResult) <- SqlRender::snakeCaseToCamelCase(toupper(colnames(tempResult)))
          } else {
            colnames(tempResult) <- toupper(colnames(tempResult))
          }
          tempResult <- convertFields(dbms(connection), tempResult)
          tempResult
        } else {
          # Execute other statements using lowLevelExecuteSql
          lowLevelExecuteSql(
            connection,
            sqlStatements[i]
          )
          NULL  # No result to process for these statements
        }
      },
      error = function(err) {
        # Log errors for each statement
        .createErrorReport(dbms(connection), err$message, sqlStatements[i], errorReportFile)
        NULL  # Continue to the next statement in case of error
      }
    )
    # Store the result if it's the final statement
    if (!is.null(result)) {
      finalResult <- result
    }
  }
